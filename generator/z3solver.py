...
def solve(puzzle: list, solve_timeout_ms: int = None, check_timeout_ms: int = None) -> None:
    n = len(puzzle)

    if solve_timeout_ms is None:
        timeout_ms = int(245*1.17**n)
    ...
    if result == sat:
        ...
        for i in range(n):
            for j in range(n):
                if sat_model[i][j]:
                    diff.append(Not(colored[i][j]))
                else:
                    diff.append(colored[i][j])
        s.add(Or(*diff))

        unique = s.check() == unsat
